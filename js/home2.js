      ...
      for(var z=0;z < 10;z ++){
        ...
        for(var x= 0;x < 38;x ++){
          var _tt = spritesArr[x + (z*38)];
            ...
            _tt.scale.x = _tt.scale.y = (_zz)*(0.35 + Math.cos(mc/5)*0.037) + 0.16;
            _tt.position.x = (x - 19)* (45 + ((_zz)*82)) + (Math.sin(mc/9)*((z+1)*80))+(Math.cos(mc/5)*(z+1)*35) + (((z+2)*0.2)*120) + WIDTH/2;
            _tt.position.y = Math.sin(mc + (x * 0.43)+((z+1)*0.3) ) * (11 + (z+1)*(3 + (Math.sin(mc/6)+0.2)*42)) + ((_zz) * (40+Math.cos(mc/9)*92)) + HEIGHT *0.3;
          }
        }

      mc+=0.004 + (Math.sin(speed)+1)* 0.03;
      speed +=0.007;
      // render the stage
